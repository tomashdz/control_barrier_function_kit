...
% Initial conditions:
x_r = [0 ; 0; 0]; % ego car
x_o = {[-7;1;u1d+0.1*randn],[-5;1;u1d+0.1*randn],[-1; 1;u1d+0.1*randn]};
x_o_hat = x_o;
% ,[2.5;1;u1d+0.1*randn],[5.5;1;u1d+0.1*randn] ,[-10;2;u1d+0.1*randn],  [-7;2;u1d+0.1*randn], [-1;2;u1d+0.1*randn], [0.45;2;u1d+0.1*randn], [5;2;u1d+0.1*randn], [-2;0;u1d+0.1*randn], [3;0;u1d+0.1*randn], [-.5; 3;u1d+0.1*randn], [3;3;u1d+0.1*randn],[-4;3;u1d+0.1*randn]}; %traffic participants
for i = 1:length(x_o)
    P{i} = eye(6); % riccati states
    x_o_hat{i}(end) = u1d;
end
...
f = @(x)  [0 ;0 ; 0; x(6); 0; u1d+(x(4)-x(1))*exp(10*((x(1)-x(4))^2+(x(2)-x(5))^2))-x(6)]; 
g = @(x) [cos(x(3)) -l*sin(x(3)); sin(x(3)) l*cos(x(3)); 0 1; 0 0; 0 0; 0 0];
f_r = @(x,u) f(x)+g(x)* [u(1); u(2)];
F_r = f_r([x_r_s x_o_s], u_s);

G = 0.1*eye(6);
D = 0.1*eye(5);
R = D*D';
Q = G*G'; 
C = eye(length([x_r_s x_o_s]));
C = C(1:end-1,:);
K = P_s*C'/R;

A = jacobian(F_r,[x_r_s x_o_s]);
dP = A*P_s+P_s*A'+Q-K*C*P_s;              % Danil: Identity might be small for NL
% dp = @(p) dP(p)
...
for i = 1:length(Unsafes)
    ...
    CBF2der = vpa(jacobian(CBFder,[x_r_s x_o_s]));
    ConstCond = vpa(CBFder*f([x_r_s x_o_s])+ 0.5* trace( D'*K'*CBF2der*K*D)+err{i}*norm(CBFder*K*C) );
    multCond = vpa(CBFder*g([x_r_s x_o_s])*u_s');
    str1 = char(ConstCond);
    str2 = char(multCond);
...
Goalder = vpa(jacobian(Goal(x_r_s(1:2)),[x_r_s, x_o_s]));
...
dt = 0.01;
N = SimTime/dt;
dW = sqrt(dt)*randn(length([x_r_s x_o_s]),N);   % increments
dv = sqrt(dt)*randn(length([x_r_s x_o_s]),N);
...
while Goal(curr_xr)>-rG+l^2 && i<N  % Solve the quadratic programs (QP) at each iteration to find an optimal risk bounded control until reaching the goal set    
    ...
    for j = 1:length(x_o)
        Dists(j) = Unsafes{j}(curr_xr,  x_o_hat{j}(:,i));
    ...
    for j = 1:length(UnsafeList)
        ...
        A(2*j-1,[1:length(u_s) length(u_s)+j]) = [multCondFun{UnsafeList(j)}(curr_xr,  x_o_hat{UnsafeList(j)}(:,i),[1 0]) multCondFun{UnsafeList(j)}(curr_xr,  x_o_hat{UnsafeList(j)}(:,i),[0 1]) -1]; % multiplier of u , bi
        b(2*j-1) = -ai* CBF{UnsafeList(j)}(curr_xr,  x_o_hat{UnsafeList(j)}(:,i))- ConstCondFun{UnsafeList(j)}(curr_xr, x_o_hat{UnsafeList(j)}(:,i), P{UnsafeList(j)}(:,:,i), err{UnsafeList(j)}(i));
        % Constraints on bi to satisfy pi risk  (Eq(17))
        A(2*j,length(u_s)+j) = 1;
        b(2*j) = min(ai, -1/T*log((1-p(UnsafeList(j)))/(1-CBF{UnsafeList(j)}(curr_xr,  x_o_hat{UnsafeList(j)}(:,i)))));
    end
    ...
    curr_xr = curr_xr + f_r(curr_xr,curr_u)*dt + G*dW(:,i);
