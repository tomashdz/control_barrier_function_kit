...
while Goal(curr_xr)>-rG+l^2 && i<N  % Solve the quadratic programs (QP) at each iteration to find an optimal risk bounded control until reaching the goal set    
    ...
    for j = 1:length(x_o)        
        curr_xohat{j} = curr_xohat{j} + F_o(curr_xohat{j},curr_xr)*dt + K(curr_P{j})*(y{j}(:,i) - C*curr_xohat{j})*dt ; 
        x_o_hat{j}(:,i+1) = curr_xohat{j};
        
        curr_P{j} = curr_P{j} + dP(curr_P{j}, curr_xr, curr_xo{j})*dt;    
        P{j}(:,:,i+1) = curr_P{j};
        
        curr_xo{j} = curr_xo{j} + F_o(curr_xo{j},curr_xr)*dt + G*W{j}(:,i)*dt;
