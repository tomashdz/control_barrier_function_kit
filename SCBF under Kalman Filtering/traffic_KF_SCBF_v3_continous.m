...
x_o_hat = x_o;
for i = 1:length(x_o)
    P{i} = 0.1*eye(3); % riccati states
    x_o_hat{i}(end) = u1d;
    dy{i} = C*x_o{i}*dt+D*dv{i};
end
...
A = jacobian(F_o_s, x_o_s);
dP_s = (A*P_s+P_s*A'+Q-K_s*C*P_s)*dt;              % Danil: Identity might be small for NL
% dp = @(p) dP(p)
...
for i = 1:length(Unsafes)
    ...
    for j = 1:size(dP_s,1)
        for k = 1:size(dP_s,2)
            strP{j,k} = char(dP_s(j,k));
            matchPattern = sprintf('%s(\\d+)', 'x_r' );
            replacePattern = sprintf('%s\\($1\\)','x_r');
            strP{j,k} = regexprep(strP{j,k},matchPattern,replacePattern);
            matchPattern = sprintf('%s(\\d+)','x_o');
            replacePattern = sprintf('%s\\($1\\)','x_o');
            strP{j,k} = regexprep(strP{j,k},matchPattern,replacePattern);
            matchPattern = sprintf('%s(\\d+)_(\\d+)','p');
            replacePattern = sprintf('%s\\($1,$1\\)','p');
            strP{j,k} = regexprep(strP{j,k},matchPattern,replacePattern); 
            str = strcat(str, strP{j,k});
            if k<size(dP_s,2)
                str = strcat(str, ', ');
            elseif j<size(dP_s,1)
                str = strcat(str, '; ');
            end
        end
    end
    str = strcat(str, ']');
    dP = str2func(['@(p,x_r,x_o) ' str]);   
end
...
while Goal(curr_xr)>-rG+l^2 && i<N  % Solve the quadratic programs (QP) at each iteration to find an optimal risk bounded control until reaching the goal set    
    ...
    for j = 1:length(UnsafeList)
        ...
        b(2*j) = min(ai, -1/T*log((1-p(UnsafeList(j)))/(1-CBF{UnsafeList(j)}(curr_xr,  curr_xohat{UnsafeList(j)}))))+10;
    end
    ...
    for j = 1:length(x_o)        
        curr_xohat{j} = curr_xohat{j} + F_o(curr_xohat{j},curr_xr)*dt + K(curr_P{j})*(dy{j}(:,i) - C*curr_xohat{j}*dt) ; 
        x_o_hat{j}(:,i+1) = curr_xohat{j};
        
        curr_P{j} = curr_P{j} + dP(curr_P{j}, curr_xr, curr_xohat{j});    
        P{j}(:,:,i+1) = curr_P{j};
        
        curr_xo{j} = curr_xo{j} + F_o(curr_xo{j},curr_xr)*dt + G*dW{j}(:,i);
        dy{j}(:,i+1) = C*curr_xo{j}*dt + D*dv{j}(:,i);
        x_o{j}(:,i+1) = curr_xo{j};
    end
