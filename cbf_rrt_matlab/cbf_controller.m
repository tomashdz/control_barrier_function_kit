...
rrt_obj=rrt_plan(T_s_r,num_obs,l,robot,v_max,w_max);

%Predictor parameters
T_s_o = 0.2;
K = 50; %monte carlo smaples
pred_samples = 10; %prediction steps
...
while (norm(robot.current_state(1:2)-x_goal)>0.6)
    if (norm(robot.current_state(1:2)-init_goal)<2)
        robot.x_goal = x_goal;
    end
    
    %Predictor
    for k=1:num_obs
         p{k}=[p{k};env_map.round_down(10,[handles.LatestMessage.Pose(k+2).Position.X handles.LatestMessage.Pose(k+2).Position.Y])];
     end
    obs_vec.current_pose = [p{1}(end,:)', p{2}(end,:)'];
    obs_vec.current_r=[0.5; 0.5];
    ...
    G = rrt_obj.planning(0,robot,obs_vec);
