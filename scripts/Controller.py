...
class Controller(object):
    ...
    def __init__(self, connected_system, goal_func, map_info, include_radius=4):
    ...
    def cbf_controller_compute(self):
        ...
        numQPvars = len(u_s) + len(unsafe_list) + len(my_map.BF.h) + 1
        ...
        for j in range(len(unsafe_list)):
            ...
            try:
                ...
                dt = Controller[j].agent.state_traj[-5:-1][-1][0] - \
        ...
        A[len(unsafe_list)+2*len(u_s)+len(my_map.BF.h),
          0:2] = [goal_func.Lyap(x_r, [1, 0]), goal_func.Lyap(x_r, [0, 1])]
