...
dt = T[1]-T[0]
params = {'x_goal': x_goal_2, 'bad_sets': bad_sets,
...
goal_square = plt.Rectangle(
    x_goal_2-np.array([1, 1]), 1, 1, color='g', alpha=0.5)
...
dist_text_1.set_text('dist(||x - [10 5]^T||, t = 5) = '+ "{:.2f}".format(dist_to_goal_1[find_nearest(T, 5)]))

plt.axhline(y=dist_to_goal_2[find_nearest(T, 15)], color='purple')
dist_text_2 = ax2.text(0.6, 1.02, '', transform=ax2.transAxes)
dist_text_2.set_text('dist(||x - [10 0]^T||, t = 15) = '+ "{:.2f}".format(dist_to_goal_2[find_nearest(T, 15)]))

dist_text_3 = ax2.text(0, 1.10, 'F_[5,15](||x - [10 0]^T||< 5) /\ G_[5,15](||x - [10 5]^T||< 10)', transform=ax2.transAxes, fontsize=12)
